    ...
    config = {
      ProgramArguments = [
        ...
        ''
          ...
          # Retry checking Colima status up to 3 times (every 15 seconds)
          while [ "$colima_attempt" -le "$colima_retries" ]; do
            ...
            if echo "$colima_output" | grep -q "colima is not running"; then
              if [ "$colima_attempt" -lt "$colima_retries" ]; then
                ${scripts.notifyServiceScript}/bin/notify-service "Open WebUI" "Colima is not running. Retrying in 15 seconds... (Attempt $colima_attempt/$colima_retries)"
                sleep 15
          ...
          # Retry Docker command up to 3 times (every 15 seconds) if it fails
          while [ "$docker_attempt" -le "$docker_retries" ]; do
            ...
            if [ "$err" -eq 0 ]; then
              ...
              if [ "$docker_attempt" -lt "$docker_retries" ]; then
                ${scripts.notifyServiceScript}/bin/notify-service "Open WebUI" "Docker failed to start Open WebUI. Retrying in 15 seconds... (Attempt $docker_attempt/$docker_retries)\nError: $output"
                sleep 15
