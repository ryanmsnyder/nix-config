...
from utils import ORIG_PATH_XATTR, eprint
    ...
    for arg in sys.argv[1:]:
        ...
        while True:
            try:
                ...
                trashed_path = trashed_path.with_name(
                    trashed_path.name + strftime(" %I.%M.%S %p")
                )
                ...
                if path_count > 0:
                    trashed_path = trashed_path.with_name(
                        trashed_path.name + f" ({path_count})"
                    )
            else:
                break
        ...
        setxattr(
            trashed_path, ORIG_PATH_XATTR, os.fsencode(resolved_path), symlink=True
        )
